...
ARG APP_MODULE=orchestrator-agent
WORKDIR /app
...
RUN sed -i '/<extensions>/d; /<\/extensions>/d; /<extension>/d; /<\/extension>/d; /com.google.cloud.artifactregistry/d; /artifactregistry-maven-wagon/,+1d' pom.xml \
    && chmod +x ./mvnw \
