package com.shrija.asset.service;
...
import com.shrija.asset.exception.AssetAgentExecutionException;
...
@Service
public class AssetConversationService {
  private static final Logger log = LoggerFactory.getLogger(AssetConversationService.class);
  private final InMemoryRunner runner;
  private final String appName;

  public AssetConversationService(AssetAgent agent) {
    BaseAgent built = agent.build();
    this.appName = built.name();
    this.runner = new InMemoryRunner(built);
  }

  public Result converse(AssetChatRequest request) {
    ...
    try {
      ...
      log.error("Asset Agent failed for actor={} session={}", request.userId(), sessionId, ex);
      throw new AssetAgentExecutionException("Asset Agent could not process the request right now.", ex);
    }
  }
