...
import com.shrija.employee.dto.ErrorResponse;
...
/**
 * Single place where every Employee Agent REST failure is turned into a stable JSON body.
 *
 ...
 * swallowed by a catch-all {@code Exception} handler and returned as 500.
 *
 * <p>This class only shapes error responses. It does not touch the agent, the MCP toolset, the A2A
 * wiring or the {@code /api/v1/employee/chat} success path.
 */
@RestControllerAdvice
public class EmployeeAgentExceptionHandler extends ResponseEntityExceptionHandler {

  private static final Logger log = LoggerFactory.getLogger(EmployeeAgentExceptionHandler.class);

  private static final String GENERIC_MESSAGE =
      "Employee Agent could not process the request. Please retry or contact support with the traceId.";

  // ---------------------------------------------------------------------
  // Application exceptions
  // ---------------------------------------------------------------------

  /** Caller is not allowed to read/modify the requested employee record. */
  ...
  /** A downstream A2A peer agent could not be resolved or reached. */
  ...
  /**
   * MCP / Gemini / session state problems surfaced as {@code IllegalStateException}. Kept as 503 to
   * match the status the other agents in this project already return for this case.
   */
  @ExceptionHandler(IllegalStateException.class)
  public ResponseEntity<ErrorResponse> handleIllegalState(
      IllegalStateException ex, HttpServletRequest request) {
    String traceId = newTraceId();
    log.error("[{}] Dependency unavailable on {}", traceId, path(request), ex);
    return build(
        traceId,
        HttpStatus.SERVICE_UNAVAILABLE,
        "Service Unavailable",
        "A required Employee Agent dependency is currently unavailable. Please retry shortly.",
        path(request));
  }

  /** The ADK runner / Gemini call failed. */
  @ExceptionHandler(EmployeeAgentExecutionException.class)
  public ResponseEntity<ErrorResponse> handleExecution(
      EmployeeAgentExecutionException ex, HttpServletRequest request) {
    String traceId = newTraceId();
    log.error("[{}] Employee Agent execution failed on {}", traceId, path(request), ex);
    return build(
        traceId,
        HttpStatus.SERVICE_UNAVAILABLE,
        "Service Unavailable",
        safeMessage(ex, "Employee Agent could not process the request right now."),
        path(request));
  }

  /** Request thread was interrupted while waiting on the agent. */
  @ExceptionHandler(InterruptedException.class)
  public ResponseEntity<ErrorResponse> handleInterrupted(
      InterruptedException ex, HttpServletRequest request) {
    Thread.currentThread().interrupt();
    String traceId = newTraceId();
    log.error("[{}] Employee Agent request interrupted on {}", traceId, path(request), ex);
    return build(
        traceId,
        HttpStatus.SERVICE_UNAVAILABLE,
        "Service Unavailable",
        "Employee Agent request was interrupted. Please retry.",
        path(request));
  }

  /** Last resort. Never leaks the internal message or stack trace to the caller. */
  @ExceptionHandler(Exception.class)
  public ResponseEntity<ErrorResponse> handleUnexpected(Exception ex, HttpServletRequest request) {
    String traceId = newTraceId();
    log.error("[{}] Unexpected Employee Agent failure on {}", traceId, path(request), ex);
    return build(
        traceId,
