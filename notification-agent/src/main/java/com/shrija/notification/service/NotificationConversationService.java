package com.shrija.notification.service;
...
import com.shrija.notification.exception.NotificationAgentExecutionException;
...
@Service
public class NotificationConversationService {
  private static final Logger log = LoggerFactory.getLogger(NotificationConversationService.class);
  private final InMemoryRunner runner;
  private final String appName;

  public NotificationConversationService(NotificationAgent notificationAgent) {
    BaseAgent agent = notificationAgent.build();
    this.appName = agent.name();
    this.runner = new InMemoryRunner(agent);
  }

  public Result converse(NotificationChatRequest request) {
    String sessionId =
    ...
    try {
      ...
      log.error(
          "Notification Agent failed for actor={} session={}", request.userId(), sessionId, ex);
      throw new NotificationAgentExecutionException(
          "Notification Agent could not process the request right now.", ex);
    }
  }
