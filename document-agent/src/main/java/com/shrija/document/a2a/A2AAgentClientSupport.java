...
public class A2AAgentClientSupport {
  public BaseAgent connect(String baseUrl, String fallbackName, String fallbackDescription) {
    ...
    String agentCardUrl = baseUrl.endsWith("/")
        ? baseUrl + ".well-known/agent-card.json"
        : baseUrl + "/.well-known/agent-card.json";
    try {
      AgentCard card = new A2ACardResolver(new JdkA2AHttpClient(), baseUrl, agentCardUrl).getAgentCard();
      Client client = Client.builder(card)
          .withTransport(JSONRPCTransport.class, new JSONRPCTransportConfig())
          .clientConfig(new ClientConfig.Builder().setStreaming(card.capabilities().streaming()).build())
          .build();
