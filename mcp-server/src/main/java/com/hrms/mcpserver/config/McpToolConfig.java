...
/**
 * Registers every domain Tools class as one flat MCP tool pool, exposed over a single SSE
 * endpoint. Each agent connects to this same server and applies its own client-side
 * ALLOWED_TOOLS allow-list (see each agent's *McpClient) to pick only the tool names relevant to
 * its role, so a tool defined once here (e.g. LeaveTools#decideOnLeaveRequest) can be shared by
 * more than one agent (Leave Agent and Manager Agent) without duplication.
 */
@Configuration
