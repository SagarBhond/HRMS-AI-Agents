...
public class PayrollTools {
  ...
  public PayrollTools(PayrollRepository salarySlipRepository, AttendanceTools attendanceTools, LeaveTools leaveTools) {
  ...
  public Payroll generateSalarySlip(Long employeeId, int month, int year, double basicSalary, double allowances, double perDayRate) {
    double unpaidLeaveDeduction = leaveTools.getUnpaidLeaveDaysForMonth(employeeId, month, year) * perDayRate;
    double deductions = unpaidLeaveDeduction;
    double netSalary = basicSalary + allowances - deductions;
    Payroll slip = salarySlipRepository.findByEmployeeIdAndMonthAndYear(employeeId, month, year).orElseGet(Payroll::new);
    slip.setEmployeeId(employeeId); slip.setMonth(month); slip.setYear(year);
    slip.setBasicSalary(basicSalary); slip.setAllowances(allowances);
    slip.setUnpaidLeaveDeduction(unpaidLeaveDeduction); slip.setDeductions(deductions);
    slip.setNetSalary(netSalary); slip.setStatus(Payroll.PayrollStatus.GENERATED);
  ...
  public Payroll getSalarySlip(Long employeeId, int month, int year) {
    return salarySlipRepository.findByEmployeeIdAndMonthAndYear(employeeId, month, year)
        .orElseThrow(() -> new IllegalArgumentException("No salary slip found for employee " + employeeId + " for " + month + "/" + year));
  }
  ...
  public double calculateDeductions(double tax, double providentFund, double insurance, double otherDeductions) {
    return Math.max(0, tax) + Math.max(0, providentFund) + Math.max(0, insurance) + Math.max(0, otherDeductions);
  }

  @Tool(description = "Calculate overtime pay")
  public double calculateOvertimePay(double overtimeHours, double hourlyRate, double overtimeMultiplier) {
  ...
  @Tool(description = "Calculate total reimbursement across expense categories for payroll inclusion")
  public double calculatePayrollReimbursement(double travel, double meals, double medical, double other) {
  ...
  public List<Payroll> getPayrollHistory(Long employeeId) {
    return salarySlipRepository.findAll().stream().filter(p -> employeeId.equals(p.getEmployeeId())).toList();
  }
  ...
  public PayrollSummary getPayrollSummary(Long employeeId, int month, int year) {
    ...
    return new PayrollSummary(employeeId, month, year, p.getBasicSalary(), p.getAllowances(),
        p.getDeductions(), p.getUnpaidLeaveDeduction(), p.getNetSalary(), p.getStatus().name());
  }
  ...
  public String explainSalary(Long employeeId, int month, int year) {
    Payroll p = getSalarySlip(employeeId, month, year);
    return "Gross components: basic=" + p.getBasicSalary() + ", allowances=" + p.getAllowances()
        + ". Deductions=" + p.getDeductions() + ", including unpaid-leave deduction="
        + p.getUnpaidLeaveDeduction() + ". Net salary=" + p.getNetSalary() + ".";
  }

  public record PayrollSummary(Long employeeId, int month, int year, double basicSalary, double allowances,
                               double deductions, double unpaidLeaveDeduction, double netSalary, String status) {}
