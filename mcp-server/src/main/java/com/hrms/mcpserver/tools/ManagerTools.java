 ...
 * Employee/Attendance/Leave/Payroll/Performance data; the underlying leave decision itself is made
 * via LeaveTools#decideOnLeaveRequest, which the Manager Agent is separately allow-listed to call.
 */
@Component
public class ManagerTools {
  ...
  public List<LeaveRequest> getPendingApprovals(
    ...
    return getTeamMembers(managerEmployeeId).stream()
        .flatMap(
            e ->
                leaveTools
                    .getLeaveRequests(e.getEmployeeId(), LeaveRequest.LeaveStatus.PENDING)
                    .stream())
        .toList();
  }

  @Tool(
      description = "Generate an attendance report for a manager's whole team on a given work date")
            ...
            .filter(
                a ->
                    a.getStatus() == com.hrms.mcpserver.domain.Attendance.AttendanceStatus.PRESENT
                        || a.getStatus()
                            == com.hrms.mcpserver.domain.Attendance.AttendanceStatus.HALF_DAY)
            .count();
    return new TeamOverview(
        managerEmployeeId, headcount, pendingLeave, present, headcount - present);
  }

  @Tool(
      description =
          "Get a summary of the manager's team leave: pending, approved, and on-leave-today counts")
  public TeamLeaveSummary getTeamLeaveSummary(
      @ToolParam(description = "Manager employee ID") Long managerEmployeeId) {
    ...
    long approvedTotal =
        team.stream()
            .flatMap(
                e ->
                    leaveTools
                        .getLeaveRequests(e.getEmployeeId(), LeaveRequest.LeaveStatus.APPROVED)
                        .stream())
            .count();
    long onLeaveToday =
        team.stream()
  ...
  @Tool(
      description =
          "Get an attendance summary (percentage, present/working days) for the manager's whole team for a month")
  ...
  public List<String> getTeamAlerts(
      @ToolParam(description = "Manager employee ID") Long managerEmployeeId) {
    ...
    for (Employee e : getTeamMembers(managerEmployeeId)) {
      AttendanceTools.AttendancePercentage pct =
          attendanceTools.calculateAttendancePercentage(
              e.getEmployeeId(), now.getMonthValue(), now.getYear());
      if (pct.percentage() < 90) {
        alerts.add(
            e.getFirstName()
                ...
                + " (id "
                + e.getEmployeeId()
                + ") attendance is below 90% this month");
      }
    }
    return alerts;
  }
  ...
  public PendingActions getPendingActions(
    ...
    return new PendingActions(
        getPendingApprovals(managerEmployeeId), getTeamAlerts(managerEmployeeId));
  }
            ...
            e -> {
              try {
                return java.util.stream.Stream.of(
                    payrollTools.getPayrollSummary(e.getEmployeeId(), month, year));
