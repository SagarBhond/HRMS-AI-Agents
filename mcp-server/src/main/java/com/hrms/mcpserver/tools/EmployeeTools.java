...
public class EmployeeTools {
  ...
  public List<Employee> listEmployees(@ToolParam(description = "Department or null") String department) {
  ...
  public List<Employee> searchEmployee(@ToolParam(description = "Name or email search text") String query) {
    String q = query == null ? "" : query.trim().toLowerCase();
    return employeeRepository.findAll().stream()
        .filter(e -> (e.getFirstName() != null && e.getFirstName().toLowerCase().contains(q))
  ...
  @Tool(description = "Update employee contact details. Contact fields are represented by email in the current model.")
  ...
  public List<Employee> getReportingHierarchy(@ToolParam(description = "Employee ID") Long employeeId) {
  ...
  @Tool(description = "Get documents associated with an employee. Returns document metadata from the employee record when available.")
  public List<EmployeeDocument> getEmployeeDocuments(@ToolParam(description = "Employee ID") Long employeeId) {
    getEmployeeProfile(employeeId);
    return List.of();
  }

  @Tool(description = "Get employment history. Lifecycle events are maintained by the HR domain; this method returns a compact master-data history placeholder.")
  public List<EmploymentHistoryItem> getEmploymentHistory(@ToolParam(description = "Employee ID") Long employeeId) {
    Employee employee = getEmployeeProfile(employeeId);
    return List.of(new EmploymentHistoryItem(employee.getEmployeeId(), employee.getDesignation(),
        employee.getDepartment(), employee.getDateOfJoining(), employee.getStatus() == null ? null : employee.getStatus().name()));
  }

  @Tool(description = "Get a compact employee summary")
  public EmployeeSummary getEmployeeSummary(@ToolParam(description = "Employee ID") Long employeeId) {
    Employee e = getEmployeeProfile(employeeId);
    return new EmployeeSummary(e.getEmployeeId(), e.getFirstName(), e.getLastName(), e.getEmail(),
        e.getDepartment(), e.getDesignation(), e.getStatus() == null ? null : e.getStatus().name());
  }

  public record EmployeeDocument(Long employeeId, String documentName, String documentType) {}
  public record EmploymentHistoryItem(Long employeeId, String designation, String department,
                                      java.time.LocalDate effectiveDate, String status) {}
  public record EmployeeSummary(Long employeeId, String firstName, String lastName, String email,
