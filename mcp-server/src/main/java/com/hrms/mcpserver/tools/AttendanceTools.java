...
public class AttendanceTools {
  ...
  @Tool(description = "Record a check-in for an employee for the current time")
  public Attendance checkIn(
    ...
    LocalTime checkInTime = now.toLocalTime().withSecond(0).withNano(0);
  ...
  @Tool(
          description =
                  "Record a check-out for an employee using the current time and compute hours worked")
  public Attendance checkOut(
    ...
    LocalDate workDate = now.toLocalDate();
    LocalTime checkOutTime = now.toLocalTime().withSecond(0).withNano(0);
    ...
    if (attendance.getCheckIn() == null) {
      throw new IllegalStateException(
              "Employee has no check-in record for " + workDate);
    }

    if (attendance.getCheckOut() != null) {
      throw new IllegalStateException(
              "Employee has already checked out for " + workDate);
    }

    attendance.setCheckOut(checkOutTime);

    double hours =
            Duration.between(attendance.getCheckIn(), checkOutTime).toMinutes() / 60.0;
  ...
  public List<Attendance> getTeamAttendance(
    ...
    return attendanceRepository.findByEmployeeIdInAndWorkDate(teamEmployeeIds, workDate);
  }
  ...
  public OvertimeResult getOvertime(
    ...
    return new OvertimeResult(
        employeeId, workDate, hours.overtimeMinutes(), hours.overtimeMinutes() / 60.0);
  }

  public record OvertimeResult(
      Long employeeId, LocalDate workDate, long overtimeMinutes, double overtimeHours) {}
  ...
  public AttendancePercentage calculateAttendancePercentage(
    ...
    long working =
        records.stream()
            .filter(
                a ->
                    a.getStatus() != Attendance.AttendanceStatus.HOLIDAY
                        && a.getStatus() != Attendance.AttendanceStatus.WEEK_OFF)
            .count();
    long present =
        records.stream()
            .filter(
                a ->
                    a.getStatus() == Attendance.AttendanceStatus.PRESENT
                        || a.getStatus() == Attendance.AttendanceStatus.HALF_DAY)
            .count();
    return new AttendancePercentage(
        employeeId, month, year, present, working, working == 0 ? 0 : present * 100.0 / working);
  }
  ...
  public List<Attendance> getLateArrivals(
    ...
    List<Attendance> source =
        employeeId == null
            ? attendanceRepository.findAll().stream()
                .filter(
                    a -> !a.getWorkDate().isBefore(startDate) && !a.getWorkDate().isAfter(endDate))
                .toList()
            : getAttendanceForRange(employeeId, startDate, endDate);
    return source.stream()
        .filter(a -> a.getCheckIn() != null && a.getCheckIn().isAfter(expectedCheckIn))
        .toList();
  }
  ...
  public List<Attendance> getEarlyDepartures(
    ...
    return getAttendanceForRange(employeeId, startDate, endDate).stream()
        .filter(a -> a.getCheckOut() != null && a.getCheckOut().isBefore(expectedCheckOut))
        .toList();
  }
  ...
  public List<AttendancePercentage> getAttendanceTrend(
    ...
    for (int i = Math.max(months, 1) - 1; i >= 0; i--) {
      ...
      result.add(
          calculateAttendancePercentage(employeeId, current.getMonthValue(), current.getYear()));
    }
    return result;
  }
  ...
  public TeamAttendanceReport generateTeamAttendanceReport(
    ...
    return new TeamAttendanceReport(
        managerEmployeeId, workDate, getTeamAttendance(managerEmployeeId, workDate));
  }
  ...
  public record TeamAttendanceReport(
      Long managerEmployeeId, LocalDate workDate, List<Attendance> records) {}
}
