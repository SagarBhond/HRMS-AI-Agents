...
/**
 * Tools backing the Compliance Agent — read-only guardrail checks consulted by other agents
 * before exposing sensitive data or performing sensitive operations. Does not own its own
 * business records; it evaluates requests against a small set of configured rules.
 */
@Component
public class ComplianceTools {
  ...
  public AccessDecision validateAccess(
      @ToolParam(description = "Role of the requester: EMPLOYEE, MANAGER, HR, ADMIN") String requesterRole,
      ...
      @ToolParam(description = "Resource being accessed, e.g. PROFILE, SALARY, LEAVE") String resource) {
    boolean isSelf =
  ...
  public ComplianceCheckResult checkPolicyCompliance(
    ...
    boolean compliant = policyName != null && !policyName.isBlank() && action != null && !action.isBlank();
    return new ComplianceCheckResult(
        compliant, compliant ? "Action satisfies " + policyName : "Missing policy name or action for compliance check");
  }
  ...
  public SensitivityResult checkSensitiveOperation(
    ...
    return new SensitivityResult(action, sensitive,
        sensitive ? "Action requires elevated authorization and audit logging" : "Action is not flagged as sensitive");
  }

  @Tool(description = "Validate whether returning a given set of fields would expose restricted data")
    ...
    List<String> requested =
        fields == null
            ...
            : java.util.Arrays.stream(fields.split(","))
                ...
                .filter(s -> !s.isBlank())
                .toList();
    List<String> flagged = requested.stream().filter(f -> RESTRICTED_FIELDS.contains(f.toLowerCase())).toList();
    return new DataExposureResult(flagged.isEmpty(), flagged);
  }

  @Tool(description = "Check whether a document type is subject to compliance retention/handling rules")
  public DocumentComplianceResult checkDocumentCompliance(
      @ToolParam(description = "Document type, e.g. CONTRACT, PAYSLIP, ID_PROOF") String documentType) {
    boolean regulated =
        documentType != null
